  ...
  if (confirming) {
    return (
      <div className="bg-white rounded-lg border border-red-200 p-4 shadow-sm">
        <p className="text-sm font-semibold text-gray-900 mb-0.5">Delete inquiry?</p>
        <p className="text-xs text-gray-500 mb-4 leading-relaxed">
          <span className="font-medium text-gray-700">{inquiry.id}</span> — {inquiry.client} · {inquiry.project}
          <br />All documents, AI data, and stage progress will be permanently removed.
        </p>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={e => { stop(e); setConfirming(false); setMenuOpen(false); }}
      ...
      >
        ...
        {menuOpen && (
          <div className="absolute bottom-full right-0 mb-1 w-44 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-20">
            <button
              type="button"
              onClick={e => { stop(e); setMenuOpen(false); setConfirming(true); }}
